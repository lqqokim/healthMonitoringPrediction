...
import com.bistel.a3.portal.dao.pdm.ulsan.HealthMapper;
import com.bistel.a3.portal.dao.pdm.ulsan.OverallMinuteTrxMapper;
import com.bistel.a3.portal.dao.pdm.ulsan.ReportMapper;
...
public class HealthService implements IHealthService {
    ...
    private List<HashMap<String,Object>>  getTrendDataToPivot(String fabId, Long eqpId, Date from, Date to) throws Exception {
        ...
        HealthMapper healthMapper = SqlSessionUtil.getMapper(sessions, fabId, HealthMapper.class);
        List<HashMap<String, Object>> pdmRawData = null;
        pdmRawData = healthMapper.selectOverallByTime(eqpId,from,to);
    ...
    private HashMap<String, Object> saveModelForAutoModeler(String fabId, Long eqpId, String model,String pca_model, List<String> model_params, long fromDate, long toDate,String userId,String description,String create_type_cd,double alarm_spec,double warn_spec) {
        ...
        List<HealthModel> result = healthMapper.selectModel(eqpId);
        if(result.size()>0){
            healthMapper.updateHealthModel(eqpId,model,pca_model,new Date(fromDate),new Date(toDate),userId,description,create_type_cd,alarm_spec,warn_spec,sParams);
            List<HashMap<String,Object>> historyResult = healthMapper.selectMaxVersionHealthModelHst(eqpId);
            if(historyResult.get(0)!=null){
                long version = Long.valueOf( historyResult.get(0).get("VERSION").toString())+1;
                healthMapper.insertHealthModelHst(eqpId,model,pca_model,new Date(fromDate),new Date(toDate),userId,description,create_type_cd,alarm_spec,warn_spec,sParams ,version);
            }else{
                healthMapper.insertHealthModelHst(eqpId,model,pca_model,new Date(fromDate),new Date(toDate),userId,description,create_type_cd,alarm_spec,warn_spec,sParams ,(long)1);
            }

        }else{
            healthMapper.insertHealthModel(eqpId,model,pca_model,new Date(fromDate),new Date(toDate),userId,description,create_type_cd,alarm_spec,warn_spec,sParams);
            healthMapper.insertHealthModelHst(eqpId,model,pca_model,new Date(fromDate),new Date(toDate),userId,description,create_type_cd,alarm_spec,warn_spec,sParams ,(long)1);
        }
        return null;
    }
...
//            System.out.println(new Matrix(preDicResult.getT2Distance()));
    ...
    public void saveHealth(String fabId, HealthModel model, List<List<Object>> data) {
        HealthMapper healthIndexMapper = SqlSessionUtil.getMapper(sessions, fabId, HealthMapper.class);
...
//                HealthIndexPDM h = makeHealthIndexPDM(model, d);
    ...
    public List<EqpWithHealthModel> getEqpsWithModel(String fabId) {
        HealthMapper healthMapper = SqlSessionUtil.getMapper(sessions, fabId, HealthMapper.class);
        return healthMapper.selectEqpsWithModel();
    }

    @Override
    public void saveHealthStatWithReport(String fabId, List<HealthStat> records) {
        HealthMapper healthMapper = SqlSessionUtil.getMapper(sessions, fabId, HealthMapper.class);
        ReportMapper reportMapper = SqlSessionUtil.getMapper(sessions, fabId, ReportMapper.class);

        PlatformTransactionManager manager = TransactionUtil.getTransactionManger(trMgrs, fabId);
        TransactionStatus status = TransactionUtil.getTransactionStatus(manager);
        try {
            for (HealthStat healthStat : records) {
                healthMapper.deleteHealthStat(healthStat);
                healthMapper.insertHealthStat(healthStat);
    ...
    public HealthStat getDailyScore(String fabId, Long eqpId, Date from, Date to, Date from90) {
        HealthMapper healthMapper = SqlSessionUtil.getMapper(sessions, fabId, HealthMapper.class);
        return healthMapper.selectScoreByEqpId(eqpId, from, to, from90);
    }

    @Override
    public HealthModel getModelByEqpId(String fabId, Long eqpId) {
        HealthMapper healthMapper = SqlSessionUtil.getMapper(sessions, fabId, HealthMapper.class);
        return healthMapper.selectModelByEqpId(eqpId);
    }

    @Override
    public List<HealthDaily> getHealth(String fabId, Long eqpId, Date from, Date to) {
        HealthMapper healthMapper = SqlSessionUtil.getMapper(sessions, fabId, HealthMapper.class);
        return healthMapper.selectHealthIndexes(eqpId, from, to);
    }
...
//            System.out.println(new Matrix(contributes));
    ...
    public List<HealthModel> getModels(String fabId) {
        HealthMapper healthMapper = SqlSessionUtil.getMapper(sessions, fabId, HealthMapper.class);
        return healthMapper.selectModels();
    }
    ...
    public HashMap<String, Object> autoModeler(Principal user,String fabId, Date fromDate, Date toDate, boolean isUnModeled, int monthRange) {
        ...
        if(isUnModeled) {
            List<HealthModel> healthModels = healthMapper.selectModels();
        ...
        List<EqpWithEtc> eqps = eqpMapper.selectList(null);
        ...
        for (int i = 0; i < eqps.size(); i++) {
            ...
            if(isUnModeled && healthModelEqpIds.containsKey(eqpId.toString())){
                ...
                try {
                    List<HashMap<String, Object>> avgs = healthMapper.selectAvgByMonthlyForMinute(eqpId, fromDate, toDate);
...
//        saveFileData(guid,dataObject);
    ...
    public HashMap<String, Object> saveSeverModel(String fabId, String userId, String dataId) {
        ...
        try {
            ...
            List<HealthModel> healthModels = healthMapper.selectModel(eqpId);

            if (healthModels.size() > 0) {
                healthMapper.updateHealthModel(eqpId, model,pcaModel, new Date(fromDate), new Date(toDate), userId, description, create_type_cd, alarm_spec, warn_spec, sParams);
                List<HashMap<String, Object>> historyResult = healthMapper.selectMaxVersionHealthModelHst(eqpId);
                if (historyResult.get(0) != null) {
                    long version = Long.valueOf(historyResult.get(0).get("VERSION").toString()) + 1;
                    healthMapper.insertHealthModelHst(eqpId, model,pcaModel, new Date(fromDate), new Date(toDate), userId, description, create_type_cd, alarm_spec, warn_spec, sParams, version);
                } else {
                    healthMapper.insertHealthModelHst(eqpId, model,pcaModel, new Date(fromDate), new Date(toDate), userId, description, create_type_cd, alarm_spec, warn_spec, sParams, (long) 1);
                }
            } else {
                healthMapper.insertHealthModel(eqpId, model,pcaModel, new Date(fromDate), new Date(toDate), userId, description, create_type_cd, alarm_spec, warn_spec, sParams);
                healthMapper.insertHealthModelHst(eqpId, model,pcaModel, new Date(fromDate), new Date(toDate), userId, description, create_type_cd, alarm_spec, warn_spec, sParams, (long) 1);
            }
        }catch (Exception e){
            e.printStackTrace();
    ...
    public DataWithSpec getDailyHealth(String fabId, Long eqpId, Long fromdate, Long todate) throws IOException, ParseException, ExecutionException, InterruptedException {
        ...
        List<HealthDaily> healthDailies = healthMapper.selectHealthIndexes(eqpId,dFromDate,dToDate);
        ...
        List<HealthModel> retVal = healthMapper.selectModel(eqpId);
        if(retVal.size()>0){
