...
public class STDMeasureTrxBinService implements IDataMigrationService {
    ...
    public List<List<Object>> getTimewaveData(String fabId, Long measurementId) {
        ...
        MeasureTrxWithBin data = mapper.selectMeasureTrxWithBinById(String.valueOf(BinDataType.TIMEWAVE.cd()), measurementId);
    ...
    public List<List<Double>> getSpectrumData(String fabId, Long measureTrxId) {
        STDTraceRawDataMapper mapper = SqlSessionUtil.getMapper(sessions, fabId, STDTraceRawDataMapper.class);
        MeasureTrxWithBin data = mapper.selectMeasureTrxWithBinById(String.valueOf(BinDataType.SPECTRUM.cd()), measureTrxId);
        if(data == null) return new ArrayList<>();
