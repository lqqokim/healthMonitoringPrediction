        ...
        try {
            if (body.length() <= 0) {
                ...
                for (ParameterSpecDataSet data : paramSpecDataList) {
                    MasterDataCache.getInstance().putAlarmWarningSpec(data.toKey(),
                            new Pair<>(data.getAlarmSpec(), data.getWarningSpec()));
                }
                ...
                for (FeatureDataSet data : paramFeatureDataList) {
                    String[] columns =
                            new String[]{Long.toString(data.getFeatureRawId()), data.getParamName(),
                                    data.getFeatureName(), data.getMainYN(), data.getAggregateYN()};
