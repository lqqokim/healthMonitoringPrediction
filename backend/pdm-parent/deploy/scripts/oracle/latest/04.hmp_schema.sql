
-- latest scripts

-- -----------------------------------------------------
-- Table CODE_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_code_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE CODE_MST_PDM (
  RAWID         NUMBER(38)      NOT NULL,
  CATEGORY      VARCHAR2(32)    NOT NULL,
  CODE          VARCHAR2(32)    NOT NULL,
  NAME          VARCHAR2(64)    NOT NULL,
  USED_YN       CHAR(1)         DEFAULT 'Y',
  DEFAULT_YN    CHAR(1)         DEFAULT 'N',
  ORDERING      NUMBER(5)       NULL,
  DESCRIPTION   VARCHAR2(2048)  NULL,
  CREATE_BY     VARCHAR2(32)    NULL,
  CREATE_DTTS   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY     VARCHAR2(32)    NULL,
  UPDATE_DTTS   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_CODE_MST_PDM ON CODE_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_CODE_MST_PDM ON CODE_MST_PDM (CATEGORY, CODE, NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table AREA_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_area_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE AREA_MST_PDM (
  RAWID         NUMBER(38)      NOT NULL,
  NAME          VARCHAR2(128)   NOT NULL,
  DESCRIPTION   VARCHAR2(2048)  NULL,
  PARENT_RAWID  NUMBER(38)      DEFAULT 0,
  CREATE_BY     VARCHAR2(32)    NULL,
  CREATE_DTTS   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY     VARCHAR2(32)    NULL,
  UPDATE_DTTS   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_AREA_MST_PDM ON AREA_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_AREA_MST_PDM ON AREA_MST_PDM (NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_MST_PDM (
  RAWID                 NUMBER(38)      NOT NULL,
  AREA_MST_RAWID        NUMBER(38)      NOT NULL,
  MODEL_NAME            VARCHAR2(128)   NOT NULL,
  NAME                  VARCHAR2(128)   NOT NULL,
  DATA_TYPE_CD          VARCHAR2(32)    DEFAULT 'STD',
  DESCRIPTION           VARCHAR2(2048)  NULL,
  IMAGE                 BLOB            NULL,
  OFFLINE_YN            CHAR(1)         DEFAULT 'N',
  CREATE_BY             VARCHAR2(32)    NULL,
  CREATE_DTTS           TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY             VARCHAR2(32)    NULL,
  UPDATE_DTTS           TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_EQP_MST_PDM ON EQP_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_MST_PDM ON EQP_MST_PDM (AREA_MST_RAWID, NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table BEARING_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_bearing_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE BEARING_MST_PDM (
  RAWID           NUMBER(38)        NOT NULL,
  MODEL_NUMBER    VARCHAR2(128)     NOT NULL,
  MANUFACTURE     VARCHAR2(128)     NOT NULL,
  BPFO            NUMBER(8,2)       NULL,
  BPFI            NUMBER(8,2)       NULL,
  BSF             NUMBER(8,2)       NULL,
  FTF             NUMBER(8,2)       NULL,
  DESCRIPTION     VARCHAR2(2048)    NULL,
  CREATE_BY       VARCHAR2(32)      NULL,
  CREATE_DTTS     TIMESTAMP         DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY       VARCHAR2(32)      NULL,
  UPDATE_DTTS     TIMESTAMP         DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_BEARING_MST_PDM ON BEARING_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_BEARING_MST_PDM ON BEARING_MST_PDM (MODEL_NUMBER, MANUFACTURE) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table PARTS_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_parts_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARTS_MST_PDM (
  RAWID               NUMBER(38)        NOT NULL,
  EQP_MST_RAWID       NUMBER(38)        NOT NULL,
  BEARING_MST_RAWID   NUMBER(38)        NULL,
  NAME                VARCHAR2(256)     NOT NULL,
  PARTS_TYPE_CD       VARCHAR2(32)      NOT NULL,
  RPM                 NUMBER(5, 2)      NULL,
  RATIO               NUMBER(5, 2)      NOT NULL,
  BASE_RATIO_YN       CHAR(1)           DEFAULT 'N',
  CREATE_BY           VARCHAR2(32)      NULL,
  CREATE_DTTS         TIMESTAMP         DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY           VARCHAR2(32)      NULL,
  UPDATE_DTTS         TIMESTAMP         DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_PARTS_MST_PDM ON PARTS_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARTS_MST_PDM ON PARTS_MST_PDM (EQP_MST_RAWID, NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table PARAM_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_MST_PDM (
  RAWID                         NUMBER(38)      NOT NULL,
  EQP_MST_RAWID                 NUMBER(38)      NOT NULL,
  PARTS_MST_RAWID               NUMBER(38)      NULL,
  SVID                          VARCHAR2(128)   NOT NULL,
  NAME                          VARCHAR2(256)   NOT NULL,
  DESCRIPTION                   VARCHAR2(2048)  NULL,
  PARAM_TYPE_CD                 VARCHAR2(8)     NOT NULL,
  TIMEWAVE_TYPE_CD              VARCHAR2(8)     NULL,
  DATA_TYPE_CD                  VARCHAR2(4)     NULL,
  UNIT_CD                       VARCHAR2(32)    NOT NULL,
  SUMMARY_YN                    CHAR(1)         DEFAULT 'Y',
  USE_YN                        CHAR(1)         DEFAULT 'Y',
  CREATE_BY                     VARCHAR2(32)    NULL,
  CREATE_DTTS                   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY                     VARCHAR2(32)    NULL,
  UPDATE_DTTS                   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_PARAM_MST_PDM ON PARAM_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_MST_PDM ON PARAM_MST_PDM (EQP_MST_RAWID, NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_TRACE_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_trace_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_TRACE_TRX_PDM (
  RAWID                 NUMBER(38)      NOT NULL,
  EQP_MST_RAWID         NUMBER(38)      NOT NULL,
  PROCESS_CONTEXT       VARCHAR2(32),
  PARAM_TYPE_CD         VARCHAR2(8),
  STATUS_CD             CHAR(1),
  DATA_COUNT            NUMBER(10),
  FILE_DATA             BLOB,
  TRACE_DTTS            TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (TRACE_DTTS)
   (PARTITION P_201712 VALUES LESS THAN (to_timestamp('201801', 'YYYYMM')),
  PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
  PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
  PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
  PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
  PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
  PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
  PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
  PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
  PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
  PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')));

CREATE UNIQUE INDEX PK_EQP_TRACE_TRX_PDM ON EQP_TRACE_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_TRACE_TRX_PDM ON EQP_TRACE_TRX_PDM (TRACE_DTTS, EQP_MST_RAWID) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_ARRAY_TRACE_TRX_PDM
-- -----------------------------------------------------
--CREATE SEQUENCE seq_eqp_array_trace_trx_pdm
--       INCREMENT BY 1
--       NOMINVALUE
--       NOMAXVALUE
--       CACHE 20
--       NOCYCLE
--       NOORDER;
--
--CREATE TABLE EQP_ARRAY_TRACE_TRX_PDM (
--  RAWID                 NUMBER(38)      NOT NULL,
--  EQP_MST_RAWID         NUMBER(38)      NOT NULL,
--  SVID                  VARCHAR2(128)   NOT NULL,
--  PROCESS_CONTEXT       VARCHAR2(32),
--  PARAM_TYPE_CD         VARCHAR2(8),
--  STATUS_CD             CHAR(1),
--  DATA_COUNT            NUMBER(10),
--  TIME_ARRAY            CLOB,
--  VALUE_ARRAY           CLOB,
--  UPPER_ALARM_SPEC      FLOAT           NULL,
--  UPPER_WARNING_SPEC    FLOAT           NULL,
--  TARGET                FLOAT           NULL,
--  LOWER_ALARM_SPEC      FLOAT           NULL,
--  LOWER_WARNING_SPEC    FLOAT           NULL,
--  TRACE_DTTS            TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
--)
--TABLESPACE NPDM_DAT
--PARTITION BY RANGE (TRACE_DTTS)
--   (PARTITION P_201712 VALUES LESS THAN (to_timestamp('201801', 'YYYYMM')),
--  PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
--  PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
--  PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
--  PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
--  PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
--  PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
--  PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
--  PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
--  PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
--  PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')));
--
--CREATE UNIQUE INDEX PK_EQP_ARRAY_TRACE_TRX_PDM ON EQP_ARRAY_TRACE_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
--CREATE UNIQUE INDEX UK_EQP_ARRAY_TRACE_TRX_PDM ON EQP_ARRAY_TRACE_TRX_PDM (TRACE_DTTS, EQP_MST_RAWID, SVID) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table VIBRATION_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_vibration_trx_pdm
    INCREMENT BY 1
    NOMINVALUE
    NOMAXVALUE CACHE 20
    NOCYCLE
    NOORDER;

CREATE TABLE VIBRATION_TRACE_TRX_PDM (
    RAWID                     NUMBER(38)    NOT NULL,
    PARAM_MST_RAWID           NUMBER(38)    NOT NULL,
    VALUE                     FLOAT,
    STATUS_CD                 CHAR(1),
    UPPER_ALARM_SPEC          FLOAT         NULL,
    UPPER_WARNING_SPEC        FLOAT         NULL,
    TARGET                    FLOAT         NULL,
    LOWER_ALARM_SPEC          FLOAT         NULL,
    LOWER_WARNING_SPEC        FLOAT         NULL,
    RULE_NAME                 VARCHAR2(32),
    CONDITION                 VARCHAR2(4000),
    PROCESS_CONTEXT           VARCHAR2(32),
    EVENT_DTTS                TIMESTAMP DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE(EVENT_DTTS)
(PARTITION P_201712 VALUES LESS THAN(to_timestamp('201801', 'YYYYMM')),
 PARTITION P_201801 VALUES LESS THAN(to_timestamp('201802', 'YYYYMM')),
 PARTITION P_201802 VALUES LESS THAN(to_timestamp('201803', 'YYYYMM')),
 PARTITION P_201803 VALUES LESS THAN(to_timestamp('201804', 'YYYYMM')),
 PARTITION P_201804 VALUES LESS THAN(to_timestamp('201805', 'YYYYMM')),
 PARTITION P_201805 VALUES LESS THAN(to_timestamp('201806', 'YYYYMM')),
 PARTITION P_201806 VALUES LESS THAN(to_timestamp('201807', 'YYYYMM')),
 PARTITION P_201807 VALUES LESS THAN(to_timestamp('201808', 'YYYYMM')),
 PARTITION P_201808 VALUES LESS THAN(to_timestamp('201809', 'YYYYMM')),
 PARTITION P_201809 VALUES LESS THAN(to_timestamp('201810', 'YYYYMM')),
 PARTITION P_201810 VALUES LESS THAN(to_timestamp('201811', 'YYYYMM')));

CREATE UNIQUE INDEX PK_VIBRATION_TRACE_TRX_PDM ON VIBRATION_TRACE_TRX_PDM(RAWID)TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_VIBRATION_TRACE_TRX_PDM ON VIBRATION_TRACE_TRX_PDM(EVENT_DTTS, PARAM_MST_RAWID)TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table VIBRATION_RAW_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_vibration_raw_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE VIBRATION_RAW_TRX_PDM (
  RAWID                 NUMBER(38)      NOT NULL,
  PARAM_MST_RAWID       NUMBER(38)      NOT NULL,
  VIBRATION_TRX_RAWID   NUMBER(38)      NOT NULL,
  DATA_TYPE_CD          VARCHAR2(32)    NOT NULL,
  MAX_FREQ              FLOAT           NULL,
  FREQ_COUNT            INT             NULL,
  RPM                   FLOAT           NULL,
  SAMPLING_TIME         FLOAT           NULL,
  BINARY_DATA           BLOB            NULL,
  EVENT_DTTS            TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  RESERVED_COL1         VARCHAR2(1024)  NULL,
  RESERVED_COL2         VARCHAR2(1024)  NULL,
  RESERVED_COL3         VARCHAR2(1024)  NULL,
  RESERVED_COL4         VARCHAR2(1024)  NULL,
  RESERVED_COL5         VARCHAR2(1024)  NULL
)
TABLESPACE NPDM_DAT
LOB(BINARY_DATA) STORE AS VIBRATION_RAW_TRX_PDM_BINARY (TABLESPACE NPDM_LOB)
PARTITION BY RANGE (EVENT_DTTS)
   (PARTITION P_201712 VALUES LESS THAN (to_timestamp('201801', 'YYYYMM')),
    PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')));

CREATE UNIQUE INDEX PK_VIBRATION_RAW_TRX_PDM ON VIBRATION_RAW_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_VIBRATION_RAW_TRX_PDM ON VIBRATION_RAW_TRX_PDM (EVENT_DTTS, PARAM_MST_RAWID, VIBRATION_TRX_RAWID, DATA_TYPE_CD) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table PARAM_FAULT_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_fault_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_FAULT_TRX_PDM (
  RAWID                     NUMBER(38)          NOT NULL,
  PARAM_MST_RAWID           NUMBER(38)          NOT NULL,
  PARAM_HEALTH_MST_RAWID    NUMBER(38)          NOT NULL,
  FAULT_TYPE_CD             VARCHAR2(8)         NOT NULL,
  VALUE                     FLOAT               NOT NULL,
  RULE_NAME                 VARCHAR2(32),
  CONDITION                 VARCHAR2(4000),
  FAULT_CLASS               VARCHAR2(1024 BYTE) DEFAULT 'N/A',
  UPPER_ALARM_SPEC          FLOAT               NULL,
  UPPER_WARNING_SPEC        FLOAT               NULL,
  TARGET                    FLOAT               NULL,
  LOWER_ALARM_SPEC          FLOAT               NULL,
  LOWER_WARNING_SPEC        FLOAT               NULL,
  FAULT_DTTS                TIMESTAMP           DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (FAULT_DTTS)
   (PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')),
    PARTITION P_201811 VALUES LESS THAN (to_timestamp('201812', 'YYYYMM')));

CREATE UNIQUE INDEX PK_PARAM_FAULT_TRX_PDM ON PARAM_FAULT_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_FAULT_TRX_PDM ON PARAM_FAULT_TRX_PDM (FAULT_DTTS, PARAM_MST_RAWID, PARAM_HEALTH_MST_RAWID) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table EQP_EVENT_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_event_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_EVENT_MST_PDM (
  RAWID             NUMBER(38)          NOT NULL,
  EQP_MST_RAWID     NUMBER(38)          NOT NULL,
  EVENT_ID          VARCHAR(32),
  EVENT_NAME        VARCHAR2(128)       NOT NULL,
  CREATE_BY         VARCHAR2(32)        NULL,
  CREATE_DTTS       TIMESTAMP           DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY         VARCHAR2(32)        NULL,
  UPDATE_DTTS       TIMESTAMP           DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_EQP_EVENT_MST_PDM ON EQP_EVENT_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_EVENT_MST_PDM ON EQP_EVENT_MST_PDM (EQP_MST_RAWID, EVENT_NAME) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table EQP_EVENT_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_event_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_EVENT_TRX_PDM (
  RAWID                   NUMBER(38)    NOT NULL,
  EQP_EVENT_MST_RAWID     NUMBER(38)    NOT NULL,
  EVENT_TYPE_CD           CHAR(1),
  EVENT_DTTS              TIMESTAMP     DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (EVENT_DTTS)
   (PARTITION P_201712 VALUES LESS THAN (to_timestamp('201801', 'YYYYMM')),
    PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')));

CREATE UNIQUE INDEX PK_EQP_EVENT_TRX_PDM ON EQP_EVENT_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_EVENT_TRX_PDM ON EQP_EVENT_TRX_PDM (EVENT_DTTS, EQP_EVENT_MST_RAWID) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table PARAM_FEATURE_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_feature_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_FEATURE_TRX_PDM (
  RAWID                       NUMBER(38)      NOT NULL,
  PARAM_MST_RAWID             NUMBER(38)      NOT NULL,
  EQP_EVENT_GROUP_MST_RAWID   NUMBER(38)      NOT NULL,
  BEGIN_DTTS                  TIMESTAMP       NOT NULL,
  END_DTTS                    TIMESTAMP       NOT NULL,
  PROCESS_CONTEXT             VARCHAR2(32),
  COUNT                       NUMBER(12,0)    NULL,
  MIN                         FLOAT           NULL,
  MAX                         FLOAT           NULL,
  MEDIAN                      FLOAT           NULL,
  MEAN                        FLOAT           NULL,
  STDDEV                      FLOAT           NULL,
  Q1                          FLOAT           NULL,
  Q3                          FLOAT           NULL,
  UPPER_ALARM_SPEC            FLOAT           NULL,
  UPPER_WARNING_SPEC          FLOAT           NULL,
  TARGET                      FLOAT           NULL,
  LOWER_ALARM_SPEC            FLOAT           NULL,
  LOWER_WARNING_SPEC          FLOAT           NULL
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (END_DTTS)
   (PARTITION P_201712 VALUES LESS THAN (to_timestamp('201801', 'YYYYMM')),
    PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')));


CREATE UNIQUE INDEX PK_PARAM_FEATURE_TRX_PDM ON PARAM_FEATURE_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_FEATURE_TRX_PDM ON PARAM_FEATURE_TRX_PDM (END_DTTS, PARAM_MST_RAWID) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table PARAM_HEALTH_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_health_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_HEALTH_TRX_PDM (
  RAWID                     NUMBER(38)      NOT NULL,
  PARAM_MST_RAWID           NUMBER(38)      NOT NULL,
  PARAM_HEALTH_MST_RAWID    NUMBER(38)      NOT NULL,
  STATUS_CD                 CHAR(1)         NOT NULL,
  DATA_COUNT                NUMBER(10)      NULL,
  SCORE                     FLOAT           NOT NULL,
  PROCESS_CONTEXT           VARCHAR2(32)    NULL,
  UPPER_ALARM_SPEC          FLOAT           NULL,
  UPPER_WARNING_SPEC        FLOAT           NULL,
  TARGET                    FLOAT           NULL,
  LOWER_ALARM_SPEC          FLOAT           NULL,
  LOWER_WARNING_SPEC        FLOAT           NULL,
  CREATE_DTTS               TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (CREATE_DTTS)
   (PARTITION P_201712 VALUES LESS THAN (to_timestamp('201801', 'YYYYMM')),
    PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')));

CREATE UNIQUE INDEX PK_PARAM_HEALTH_TRX_PDM ON PARAM_HEALTH_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_HEALTH_TRX_PDM ON PARAM_HEALTH_TRX_PDM (CREATE_DTTS, PARAM_MST_RAWID, PARAM_HEALTH_MST_RAWID) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table PARAM_HEALTH_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_health_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_HEALTH_MST_PDM (
  RAWID                         NUMBER(38)      NOT NULL,
  PARAM_MST_RAWID               NUMBER(38)      NOT NULL,
  HEALTH_LOGIC_MST_RAWID        NUMBER(38)      NOT NULL,
  APPLY_LOGIC_YN                CHAR(1)         DEFAULT 'Y',
  CREATE_BY                     VARCHAR2(32)    NULL,
  CREATE_DTTS                   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY                     VARCHAR2(32)    NULL,
  UPDATE_DTTS                   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_PARAM_HEALTH_MST_PDM ON PARAM_HEALTH_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_HEALTH_MST_PDM ON PARAM_HEALTH_MST_PDM (PARAM_MST_RAWID, HEALTH_LOGIC_MST_RAWID) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table PARAM_HEALTH_OPTION_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_health_opt_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_HEALTH_OPTION_MST_PDM (
  RAWID                   NUMBER(38)        NOT NULL,
  PARAM_HEALTH_MST_RAWID  NUMBER(38)        NOT NULL,
  OPTION_NAME             VARCHAR2(32)      NOT NULL,
  OPTION_VALUE            FLOAT             NOT NULL,
  DESCRIPTION             VARCHAR2(2048)    NULL,
  CREATE_BY               VARCHAR2(32)      NULL,
  CREATE_DTTS             TIMESTAMP         DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY               VARCHAR2(32)      NULL,
  UPDATE_DTTS             TIMESTAMP         DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_PARAM_HEALTH_OPTION_MST_PDM ON PARAM_HEALTH_OPTION_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_HEALTH_OPTION_MST_PDM ON PARAM_HEALTH_OPTION_MST_PDM (PARAM_HEALTH_MST_RAWID, OPTION_NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table HEALTH_LOGIC_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_health_logic_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE HEALTH_LOGIC_MST_PDM (
  RAWID               NUMBER(38)        NOT NULL,
  CATEGORY            VARCHAR2(32)      NOT NULL,
  CODE                VARCHAR2(32)      NOT NULL,
  NAME                VARCHAR2(128)     NOT NULL,
  ALARM_CONDITION     VARCHAR2(1024),
  WARNING_CONDITION   VARCHAR2(1024),
  DESCRIPTION         VARCHAR2(2048)    NULL,
  CREATE_BY           VARCHAR2(32)      NULL,
  CREATE_DTTS         TIMESTAMP         DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY           VARCHAR2(32)      NULL,
  UPDATE_DTTS         TIMESTAMP         DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_HEALTH_LOGIC_MST_PDM ON HEALTH_LOGIC_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_HEALTH_LOGIC_MST_PDM ON HEALTH_LOGIC_MST_PDM (CATEGORY, CODE) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table HEALTH_LOGIC_OPTION_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_health_logic_opt_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE HEALTH_LOGIC_OPTION_MST_PDM (
  RAWID                   NUMBER(38)        NOT NULL,
  HEALTH_LOGIC_MST_RAWID  NUMBER(38)        NOT NULL,
  OPTION_NAME             VARCHAR2(32)      NOT NULL,
  OPTION_VALUE            FLOAT             NOT NULL,
  DESCRIPTION             VARCHAR2(2048)    NULL,
  CREATE_BY               VARCHAR2(32)      NULL,
  CREATE_DTTS             TIMESTAMP         DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY               VARCHAR2(32)      NULL,
  UPDATE_DTTS             TIMESTAMP         DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_HEALTH_LOGIC_OPTION_MST_PDM ON HEALTH_LOGIC_OPTION_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_HEALTH_LOGIC_OPTION_MST_PDM ON HEALTH_LOGIC_OPTION_MST_PDM (HEALTH_LOGIC_MST_RAWID, OPTION_NAME) TABLESPACE NPDM_IDX;

---- -----------------------------------------------------
---- Table ALARM_SMTP_CFG_PDM
---- -----------------------------------------------------
--CREATE SEQUENCE seq_fault_smtp_cfg_pdm
--       INCREMENT BY 1
--       NOMINVALUE
--       NOMAXVALUE
--       CACHE 20
--       NOCYCLE
--       NOORDER;
--
--CREATE TABLE FAULT_SMTP_CFG_PDM (
--  HOST              VARCHAR2(1024)  NOT NULL,
--  PORT              NUMBER(10,0)    NOT NULL,
--  USER_ID           VARCHAR2(64)    NOT NULL,
--  PASSWORD          VARCHAR2(1024)  NOT NULL,
--  SSL               CHAR(1)         DEFAULT 'Y',
--  FROM_ADDR         VARCHAR2(1024)  NOT NULL,
--  TO_ADDR           VARCHAR2(2048)  NOT NULL,
--  DESCRIPTION       VARCHAR2(2048)  NULL,
--  CREATE_BY         VARCHAR2(32)    NULL,
--  CREATE_DTTS       TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
--  UPDATE_BY         VARCHAR2(32)    NULL,
--  UPDATE_DTTS       TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
--)
--TABLESPACE NPDM_DAT;


-- -----------------------------------------------------
-- Table MANUAL_RPM_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_manual_rpm_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE MANUAL_RPM_MST_PDM
(
    RAWID           NUMBER(10)      NOT NULL,
    PARAM_MST_RAWID NUMBER(10)      NOT NULL,
    RPM             NUMBER(6,2)     NULL,
    CREATE_BY       VARCHAR(32)     NULL,
	CREATE_DTTS     TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
	UPDATE_BY       VARCHAR(32)     NULL,
	UPDATE_DTTS     TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_MANUAL_RPM_MST_PDM ON MANUAL_RPM_MST_PDM (RAWID)	TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_MANUAL_RPM_MST_PDM ON MANUAL_RPM_MST_PDM (PARAM_MST_RAWID) TABLESPACE NPDM_IDX;


---- -----------------------------------------------------
---- Table TRACE_DAILY_SUM_PDM
---- -----------------------------------------------------
--CREATE TABLE TRACE_DAILY_SUM_PDM (
--  RAWID                 NUMBER(38) NOT NULL,
--  PARAM_MST_RAWID       NUMBER(38) NOT NULL,
--  ALARM_SPEC            FLOAT,
--  WARNING_SPEC          FLOAT,
--  AVG_PERIOD            FLOAT,
--  AVG_DAILY             FLOAT,
--  AVG_OOS               FLOAT,
--  VARIATION             FLOAT,
--  CAUSE                 VARCHAR2(2048 BYTE),
--  SUM_DTTS              TIMESTAMP(6) DEFAULT CURRENT_TIMESTAMP,
--  CREATE_BY             VARCHAR2(32) NULL,
--  CREATE_DTTS           TIMESTAMP DEFAULT CURRENT_TIMESTAMP
--)
--TABLESPACE NPDM_DAT
--PARTITION BY RANGE (SUM_DTTS)
--   (PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
--    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
--    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
--    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
--    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
--    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
--    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
--    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
--    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
--    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')),
--    PARTITION P_201811 VALUES LESS THAN (to_timestamp('201812', 'YYYYMM')));
--
--CREATE UNIQUE INDEX PK_TRACE_DAILY_SUM_PDM ON TRACE_DAILY_SUM_PDM (RAWID) TABLESPACE NPDM_IDX;
--CREATE UNIQUE INDEX UK_TRACE_DAILY_SUM_PDM ON TRACE_DAILY_SUM_PDM (SUM_DTTS, PARAM_MST_RAWID) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table EQP_ALARM_DAILY_SUM_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_fault_daily_sum_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_FAULT_DAILY_SUM_PDM (
  RAWID           NUMBER(38)    NOT NULL,
  EQP_MST_RAWID   NUMBER(38)    NOT NULL,
  FAULT_TYPE_CD   CHAR(1)       NULL
  VALUE           FLOAT         NULL,
  SUM_DTTS        TIMESTAMP(6)  DEFAULT CURRENT_TIMESTAMP,
  CREATE_BY       VARCHAR2(32)  NULL,
  CREATE_DTTS     TIMESTAMP(6)  DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (SUM_DTTS)
   (PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')),
    PARTITION P_201811 VALUES LESS THAN (to_timestamp('201812', 'YYYYMM')));

CREATE UNIQUE INDEX PK_EQP_FAULT_DAILY_SUM_PDM ON EQP_FAULT_DAILY_SUM_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_FAULT_DAILY_SUM_PDM ON EQP_FAULT_DAILY_SUM_PDM (SUM_DTTS, EQP_MST_RAWID, FAULT_TYPE_CD) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table EQP_HEALTH_DAILY_SUM_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_health_daily_sum_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_HEALTH_DAILY_SUM_PDM (
  RAWID                   NUMBER(38)    NOT NULL,
  EQP_MST_RAWID           NUMBER(38)    NOT NULL,
  PARAM_HEALTH_MST_RAWID  NUMBER(38)    NOT NULL,
  SCORE                   FLOAT         NULL,
  ALARM_COUNT             NUMBER(10, 0) NULL,
  SUM_DTTS                TIMESTAMP(6)  DEFAULT CURRENT_TIMESTAMP,
  CREATE_BY               VARCHAR2(32)  NULL,
  CREATE_DTTS             TIMESTAMP(6)  DEFAULT CURRENT_TIMESTAMP,
  PREVIOUS_AVG            FLOAT,
  TODAY_AVG               FLOAT,
  DATA_COUNT			        NUMBER(38,0),
  SIGMA                   FLOAT,
  PREVIOUS_DATE           TIMESTAMP(6)
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (SUM_DTTS)
   (PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')),
    PARTITION P_201811 VALUES LESS THAN (to_timestamp('201812', 'YYYYMM')));

CREATE UNIQUE INDEX PK_EQP_HEALTH_DAILY_SUM_PDM ON EQP_HEALTH_DAILY_SUM_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_HEALTH_DAILY_SUM_PDM ON EQP_HEALTH_DAILY_SUM_PDM (SUM_DTTS, EQP_MST_RAWID, PARAM_HEALTH_MST_RAWID) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table PARAM_HEALTH_RUL_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_health_rul_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_HEALTH_RUL_TRX_PDM (
  RAWID                   NUMBER(38)    NOT NULL,
  PARAM_HEALTH_MST_RAWID  NUMBER(38)    NOT NULL,
  INTERCEPT               FLOAT         NULL,
  SLOPE                   FLOAT         NULL,
  XVALUE                  FLOAT         NULL,
  CREATE_DTTS             TIMESTAMP     DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (CREATE_DTTS)
   (PARTITION P_201712 VALUES LESS THAN (to_timestamp('201801', 'YYYYMM')),
    PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')));

CREATE UNIQUE INDEX PK_PARAM_HEALTH_RUL_TRX_PDM ON PARAM_HEALTH_RUL_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_HEALTH_RUL_TRX_PDM ON PARAM_HEALTH_RUL_TRX_PDM (CREATE_DTTS, PARAM_HEALTH_TRX_RAWID) TABLESPACE NPDM_IDX;

---- -----------------------------------------------------
---- Table BATCH_JOB_HST_PDM
---- -----------------------------------------------------
--
--CREATE TABLE BATCH_JOB_HST_PDM
--(
--    JOB_DTTS        TIMESTAMP (6) NOT NULL ENABLE,
--    EQP_ID          NUMBER(10,0),
--    JOB_CD          VARCHAR2(32 BYTE) NOT NULL ENABLE,
--    JOB_STATUS_CD   VARCHAR2(32 BYTE),
--    JOB_TYPE_CD     VARCHAR2(32 BYTE),
--    UPDATE_DTTS     TIMESTAMP (6),
--    UPDATE_BY       VARCHAR2(50 BYTE)
--)
--TABLESPACE NPDM_DAT ;
--
--create unique index UK_BATCH_JOB_HST_PDM ON BATCH_JOB_HST_PDM (JOB_DTTS, JOB_CD) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table MONITORING_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_monitoring_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE MONITORING_MST_PDM
(
    RAWID         NUMBER(38)            NOT NULL ENABLE,
    NAME          VARCHAR2(256 BYTE)    NOT NULL ENABLE,
    IMAGE         CLOB,
    DATAS         CLOB,
    WIDTH         NUMBER(10,0)          NOT NULL ENABLE,
    HEIGHT        NUMBER(10,0)          NOT NULL ENABLE,
    CREATE_BY     VARCHAR2(32 BYTE),
    CREATE_DTTS   TIMESTAMP (6)         DEFAULT CURRENT_TIMESTAMP,
    UPDATE_BY     VARCHAR2(32 BYTE),
    UPDATE_DTTS   TIMESTAMP (6)         DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_MONITORING_MST_PDM ON MONITORING_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_MONITORING_MST_PDM ON MONITORING_MST_PDM (NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table CONDITIONAL_SPEC_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_conditional_spec_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE CONDITIONAL_SPEC_MST_PDM (
  RAWID             NUMBER(38)      NOT NULL,
  MODEL_NAME        VARCHAR2(32)    NOT NULL,
  RULE_NAME         VARCHAR2(32)    NOT NULL,
  EXPRESSION        VARCHAR2(4000),
  EXPRESSION_VALUE  VARCHAR2(4000),
  CONDITION         VARCHAR2(4000),
  DESCRIPTION       VARCHAR2(2048)  NULL,
  CREATE_BY         VARCHAR2(32)    NULL,
  CREATE_DTTS       TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY         VARCHAR2(32)    NULL,
  UPDATE_DTTS       TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_CONDITIONAL_SPEC_MST_PDM ON CONDITIONAL_SPEC_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_CONDITIONAL_SPEC_MST_PDM ON CONDITIONAL_SPEC_MST_PDM (MODEL_NAME, RULE_NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table MODEL_PARAM_SPEC_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_model_param_spec_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE MODEL_PARAM_SPEC_MST_PDM (
  RAWID                         NUMBER(38)      NOT NULL,
  CONDITIONAL_SPEC_MST_RAWID    NUMBER(38)      NOT NULL,
  PARAM_NAME                    VARCHAR2(256)   NOT NULL,
  UPPER_ALARM_SPEC              FLOAT,
  UPPER_WARNING_SPEC            FLOAT,
  TARGET                        FLOAT,
  LOWER_ALARM_SPEC              FLOAT,
  LOWER_WARNING_SPEC            FLOAT,
  DESCRIPTION                   VARCHAR2(2048)  NULL,
  CREATE_BY                     VARCHAR2(32)    NULL,
  CREATE_DTTS                   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY                     VARCHAR2(32)    NULL,
  UPDATE_DTTS                   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_MODEL_PARAM_SPEC_MST_PDM ON MODEL_PARAM_SPEC_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_MODEL_PARAM_SPEC_MST_PDM ON MODEL_PARAM_SPEC_MST_PDM (CONDITIONAL_SPEC_MST_RAWID, PARAM_NAME) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_SPEC_LINK_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_spec_link_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_SPEC_LINK_MST_PDM (
  RAWID                         NUMBER(38)      NOT NULL,
  EQP_MST_RAWID                 NUMBER(38)      NOT NULL,
  CONDITIONAL_SPEC_MST_RAWID    NUMBER(38)      NOT NULL,
  ORDERING                      NUMBER(5)       DEFAULT 1,
  DESCRIPTION                   VARCHAR2(2048)  NULL,
  CREATE_BY                     VARCHAR2(32)    NULL,
  CREATE_DTTS                   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY                     VARCHAR2(32)    NULL,
  UPDATE_DTTS                   TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_EQP_SPEC_LINK_MST_PDM ON EQP_SPEC_LINK_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_SPEC_LINK_MST_PDM ON EQP_SPEC_LINK_MST_PDM (EQP_MST_RAWID, CONDITIONAL_SPEC_MST_RAWID) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table PARAM_SPEC_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_spec_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_SPEC_MST_PDM (
  RAWID                             NUMBER(38)      NOT NULL,
  PARAM_MST_RAWID                   NUMBER(38)      NOT NULL,
  EQP_SPEC_LINK_MST_RAWID           NUMBER(38)      NOT NULL,
  UPPER_ALARM_SPEC                  FLOAT,
  UPPER_WARNING_SPEC                FLOAT,
  TARGET                            FLOAT,
  LOWER_ALARM_SPEC                  FLOAT,
  LOWER_WARNING_SPEC                FLOAT,
  DESCRIPTION                       VARCHAR2(2048)  NULL,
  CREATE_BY                         VARCHAR2(32)    NULL,
  CREATE_DTTS                       TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY                         VARCHAR2(32)    NULL,
  UPDATE_DTTS                       TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_PARAM_SPEC_MST_PDM ON PARAM_SPEC_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_SPEC_MST_PDM ON PARAM_SPEC_MST_PDM (PARAM_MST_RAWID, EQP_SPEC_LINK_MST_RAWID) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_ALARM_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_dimension_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_DIMENSION_TRX_PDM
(
    RAWID                       NUMBER(38)          NOT NULL,
    PARAM_MST_RAWID             NUMBER(38)          NOT NULL,
    EQP_EVENT_GROUP_MST_RAWID   NUMBER(38)          NOT NULL,
    BEGIN_DTTS                  TIMESTAMP (6)       NOT NULL,
    END_DTTS                    TIMESTAMP (6)       NOT NULL,
    VALUE                       VARCHAR2(1024),
    PROCESS_CONTEXT             VARCHAR2(32)
) TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_PARAM_DIMENSION_TRX_PDM ON PARAM_DIMENSION_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX PK_PARAM_DIMENSION_TRX_PDM ON PARAM_DIMENSION_TRX_PDM (BEGIN_DTTS, PARAM_MST_RAWID, EQP_EVENT_GROUP_MST_RAWID) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_ALARM_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_alarm_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_ALARM_MST_PDM (
  RAWID                     NUMBER(38)          NOT NULL,
  EQP_MST_RAWID             NUMBER(38)          NOT NULL,
  ALARM_ID                  VARCHAR2(32)        NOT NULL,
  ALARM_CODE                VARCHAR2(32),
  ALAMR_TEXT                VARCHAR2(1024),
  CREATE_BY                 VARCHAR2(32)        NULL,
  CREATE_DTTS               TIMESTAMP           DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY                 VARCHAR2(32)        NULL,
  UPDATE_DTTS               TIMESTAMP           DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_EQP_ALARM_MST_PDM ON EQP_ALARM_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_ALARM_MST_PDM ON EQP_ALARM_MST_PDM (EQP_MST_RAWID, ALARM_ID, ALARM_CODE) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_ALARM_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_alarm_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_ALARM_TRX_PDM (
  RAWID                     NUMBER(38)          NOT NULL,
  EQP_ALARM_MST_RAWID       NUMBER(38)          NOT NULL,
  ALARM_ID                  VARCHAR2(32)        NOT NULL,
  ALARM_CODE                VARCHAR2(32),
  ALAMR_TEXT                VARCHAR2(1024),
  PROCESS_CONTEXT           VARCHAR2(32),
  ALARM_DTTS                TIMESTAMP           DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_EQP_ALARM_TRX_PDM ON EQP_ALARM_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_ALARM_TRX_PDM ON EQP_ALARM_TRX_PDM (EQP_ALARM_MST_RAWID, ALARM_ID, ALARM_CODE) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_EVENT_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_event_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_EVENT_MST_PDM (
  RAWID                     NUMBER(38)          NOT NULL,
  EQP_MST_RAWID             NUMBER(38)          NOT NULL,
  EVENT_ID                  VARCHAR2(512)       NOT NULL,
  EVENT_NAME                VARCHAR2(128)       NOT NULL,
  VIRTUAL_YN                CHAR(1)             DEFAULT 'N',
  VIRTUAL_LOGIC             VARCHAR2(1024)      NULL,
  CREATE_BY                 VARCHAR2(32)        NULL,
  CREATE_DTTS               TIMESTAMP           DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY                 VARCHAR2(32)        NULL,
  UPDATE_DTTS               TIMESTAMP           DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_EQP_EVENT_MST_PDM ON EQP_EVENT_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_EVENT_MST_PDM ON EQP_EVENT_MST_PDM (EQP_MST_RAWID, EVENT_ID) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table EQP_EVENT_GROUP_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_event_group_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_EVENT_GROUP_MST_PDM (
  RAWID                     NUMBER(38)          NOT NULL,
  EQP_MST_RAWID             NUMBER(38)          NOT NULL,
  EG_NAME                   VARCHAR2(128)       NOT NULL,
  START_EVENT_ID            VARCHAR2(512)       NOT NULL,
  END_EVENT_ID              VARCHAR2(512)       NOT NULL,
  EG_TYPE_CD                CHAR(1)             DEFAULT 'P',
  USE_YN                    CHAR(1)             DEFAULT 'Y',
  TIMEOUT_MS                NUMBER(32)          DEFAULT 600000,
  CREATE_BY                 VARCHAR2(32)        NULL,
  CREATE_DTTS               TIMESTAMP           DEFAULT CURRENT_TIMESTAMP,
  UPDATE_BY                 VARCHAR2(32)        NULL,
  UPDATE_DTTS               TIMESTAMP           DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_EQP_EVENT_GROUP_MST_PDM ON EQP_EVENT_GROUP_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_EVENT_GROUP_MST_PDM ON EQP_EVENT_GROUP_MST_PDM (EQP_MST_RAWID, EG_NAME) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table EQP_EVENT_GROUP_TRX_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_event_group_trx_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_EVENT_GROUP_TRX_PDM (
  RAWID                     NUMBER(38)          NOT NULL,
  EQP_EVENT_GROUP_MST_RAWID NUMBER(38)          NOT NULL,
  EG_NAME                   VARCHAR2(128)       NOT NULL,
  STATUS_CD                 CHAR(1)             DEFAULT 'I',
  PROCESS_CONTEXT           VARCHAR2(32)        NULL,
  START_EVENT_ID            VARCHAR2(512)       NOT NULL,
  END_EVENT_ID              VARCHAR2(512)       NOT NULL,
  START_DTTS                TIMESTAMP           DEFAULT CURRENT_TIMESTAMP,
  END_DTTS                  TIMESTAMP           DEFAULT CURRENT_TIMESTAMP
)
TABLESPACE NPDM_DAT
PARTITION BY RANGE (END_DTTS)
   (PARTITION P_201712 VALUES LESS THAN (to_timestamp('201801', 'YYYYMM')),
    PARTITION P_201801 VALUES LESS THAN (to_timestamp('201802', 'YYYYMM')),
    PARTITION P_201802 VALUES LESS THAN (to_timestamp('201803', 'YYYYMM')),
    PARTITION P_201803 VALUES LESS THAN (to_timestamp('201804', 'YYYYMM')),
    PARTITION P_201804 VALUES LESS THAN (to_timestamp('201805', 'YYYYMM')),
    PARTITION P_201805 VALUES LESS THAN (to_timestamp('201806', 'YYYYMM')),
    PARTITION P_201806 VALUES LESS THAN (to_timestamp('201807', 'YYYYMM')),
    PARTITION P_201807 VALUES LESS THAN (to_timestamp('201808', 'YYYYMM')),
    PARTITION P_201808 VALUES LESS THAN (to_timestamp('201809', 'YYYYMM')),
    PARTITION P_201809 VALUES LESS THAN (to_timestamp('201810', 'YYYYMM')),
    PARTITION P_201810 VALUES LESS THAN (to_timestamp('201811', 'YYYYMM')));

CREATE UNIQUE INDEX PK_EQP_EVENT_GROUP_TRX_PDM ON EQP_EVENT_GROUP_TRX_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_EVENT_GROUP_TRX_PDM ON EQP_EVENT_GROUP_TRX_PDM (START_DTTS, EQP_EVENT_GROUP_MST_RAWID) TABLESPACE NPDM_IDX;




-----------------------
--- for interface -----
-----------------------

-- -----------------------------------------------------
-- Table DATA_SYNC_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_data_sync_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE DATA_SYNC_MST_PDM
(
    RAWID                   NUMBER(38)      NOT NULL,
    DB_TYPE                 VARCHAR2(20)    NOT NULL,
    IP                      VARCHAR2(32)    NOT NULL,
    PORT                    NUMBER(5,0)     NOT NULL,
    USER_NAME               VARCHAR2(32)    NOT NULL,
    PASSWORD                VARCHAR2(64)    NOT NULL,
    DB_NAME                 VARCHAR2(32)    NOT NULL,
    SID                     VARCHAR2(32)    NULL,
    SERVICE_NAME            VARCHAR2(32)    NULL,
    AUTO_SYNC_YN            CHAR(1)         DEFAULT 'N'
    CREATE_BY               VARCHAR2(32)    NULL,
    CREATE_DTTS             TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    UPDATE_BY               VARCHAR2(32)    NULL,
    UPDATE_DTTS             TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
) TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_DATA_SYNC_MST_PDM ON DATA_SYNC_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_DATA_SYNC_MST_PDM ON DATA_SYNC_MST_PDM (DB_TYPE, IP, PORT, USER_NAME) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table EQP_SYNC_LINK_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_eqp_sync_link_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE EQP_SYNC_LINK_MST_PDM
(
    RAWID                   NUMBER(38)      NOT NULL,
    EQP_MST_RAWID           NUMBER(38)      NOT NULL,
    DATA_SYNC_MST_RAWID     NUMBER(38)      NOT NULL,
    IS_DELETED              CHAR(1)         DEFAULT 'N',
    CREATE_BY               VARCHAR2(32)    NULL,
    CREATE_DTTS             TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    UPDATE_BY               VARCHAR2(32)    NULL,
    UPDATE_DTTS             TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
) TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_EQP_SYNC_LINK_MST_PDM ON EQP_SYNC_LINK_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_EQP_SYNC_LINK_MST_PDM ON EQP_SYNC_LINK_MST_PDM (EQP_MST_RAWID, DATA_SYNC_MST_RAWID) TABLESPACE NPDM_IDX;

-- -----------------------------------------------------
-- Table ALARM_SYNC_LINK_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_alarm_sync_link_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE ALARM_SYNC_LINK_MST_PDM
(
    RAWID                   NUMBER(38)      NOT NULL,
    EQP_ALARM_MST_RAWID     NUMBER(38)      NOT NULL,
    DATA_SYNC_MST_RAWID     NUMBER(38)      NOT NULL,
    IS_DELETED              CHAR(1)         DEFAULT 'N',
    CREATE_BY               VARCHAR2(32)    NULL,
    CREATE_DTTS             TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    UPDATE_BY               VARCHAR2(32)    NULL,
    UPDATE_DTTS             TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
) TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_ALARM_SYNC_LINK_MST_PDM ON ALARM_SYNC_LINK_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_ALARM_SYNC_LINK_MST_PDM ON ALARM_SYNC_LINK_MST_PDM (EQP_ALARM_MST_RAWID, DATA_SYNC_MST_RAWID) TABLESPACE NPDM_IDX;


-- -----------------------------------------------------
-- Table PARAM_SYNC_LINK_MST_PDM
-- -----------------------------------------------------
CREATE SEQUENCE seq_param_sync_link_mst_pdm
       INCREMENT BY 1
       NOMINVALUE
       NOMAXVALUE
       CACHE 20
       NOCYCLE
       NOORDER;

CREATE TABLE PARAM_SYNC_LINK_MST_PDM
(
    RAWID                   NUMBER(38)      NOT NULL,
    PARAM_MST_RAWID         NUMBER(38)      NOT NULL,
    DATA_SYNC_MST_RAWID     NUMBER(38)      NOT NULL,
    IS_DELETED              CHAR(1)         DEFAULT 'N',
    CREATE_BY               VARCHAR2(32)    NULL,
    CREATE_DTTS             TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    UPDATE_BY               VARCHAR2(32)    NULL,
    UPDATE_DTTS             TIMESTAMP       DEFAULT CURRENT_TIMESTAMP
) TABLESPACE NPDM_DAT;

CREATE UNIQUE INDEX PK_PARAM_SYNC_LINK_MST_PDM ON PARAM_SYNC_LINK_MST_PDM (RAWID) TABLESPACE NPDM_IDX;
CREATE UNIQUE INDEX UK_PARAM_SYNC_LINK_MST_PDM ON PARAM_SYNC_LINK_MST_PDM (PARAM_MST_RAWID, DATA_SYNC_MST_RAWID) TABLESPACE NPDM_IDX;

