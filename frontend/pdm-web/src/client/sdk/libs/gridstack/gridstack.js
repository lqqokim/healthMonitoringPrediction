(function (scope, _) {
    ...
    var Utils = {
        is_intercepted: function (aNode, bNode) {
            return !(aNode.x + aNode.width <= bNode.x || bNode.x + bNode.width <= aNode.x || aNode.y + aNode.height <= bNode.y || bNode.y + bNode.height <= aNode.y);
        },
    ...
    var GridStack = function (el, opts) {
        ...
        this.opts = _.defaults(opts || {}, {
            width: parseInt(this.container.attr('data-gs-width')) || 1,
            height: parseInt(this.container.attr('data-gs-height')) || 1,
        ...
        // create GridStackEngine
        // second param is onchange callback
        ...
        // by ysyun
        this.opts.cell_height = parseInt(Math.round( this.container.height()/this.opts.height ).toFixed(0));
        //this.container.height(this.container.height());

        // call one at first
        var on_resize_handler = function () {
            // by ysyun
            // set auto resize grid styles
            self._update_styles();
        ...
        if( _ ) {
          var lazyLayout = _.debounce(on_resize_handler, 300);
    ...
    GridStack.prototype._update_styles = function (max_height) {
        ...
        // set a cell style of grid
        // usage gridstack.css px to %
        //  -------------------------------------------------------------
        //  .grid-stack-item[data-gs-height="3"]  { height: 99.99999% }
        //  .grid-stack-item[data-gs-height="2"]  { height: 66.66666% }
        //  .grid-stack-item[data-gs-height="1"]  { height: 33.33333% }

        //  .grid-stack-item[data-gs-y="2"]  { top: 66.66666% }
        //  .grid-stack-item[data-gs-y="1"]  { top: 33.33333% }
    ...
    GridStack.prototype._prepare_element = function (el) {
        ...
        el.draggable({
            ...
            scroll: true,
            //appendTo: 'body',
            // widget container can move in this area '.grid-stack-xxxx'
            containment: '.' + this.opts._class,
            snap: '.' + this.opts._class,
    ...
    GridStack.prototype.add_widget = function (el, x, y, width, height, widget_id, auto_position) {
        el = $(el);
        if (typeof x != 'undefined') el.attr('data-gs-x', x);
        if (typeof y != 'undefined') el.attr('data-gs-y', y);
        if (typeof width != 'undefined') el.attr('data-gs-width', width);
        if (typeof height != 'undefined') el.attr('data-gs-height', height);
        if (typeof widget_id != 'undefined') el.attr('data-gs-id', widget_id);
    ...
    // by ysyun
    // find available node
    GridStack.prototype.find_available_space_for_add_widget = function (layoutX, layoutY, newWidget) {
        // 1) make 2 dimension array and then init to zero
        var used = []; // rows
        ...
        // 2) set used node with 1
        //console.log('gridstack nodes length: ', this.grid.nodes.length);
        _.each(this.grid.nodes, function (node) {
            //console.log('------------------------>>>>');
            for( var y = node.y; y < (node.y + node.height); y++) {
                for( var x = node.x; x < (node.x + node.width); x++) {
                    //console.log('used['+y+']['+x+']=1');
                    used[y][x] = 1;
                }
            }
        });
        ...
        var availableNode = [];
        var loopY = layoutY - newWidget.height + 1;
        var loopX = layoutX - newWidget.width + 1;
        for (var y = 0; y < loopY; y++) {
            for(var x = 0; x < loopX; x++) {
                var sum = 0;
                for(var addX = 0; addX < newWidget.width; addX++) {
                    for(var addY = 0; addY < newWidget.height; addY++) {
                        sum += used[y+addY][x+addX];
                    }
                }
                // if sum is over 1, it is used node
                if(sum === 0) {
                    availableNode.push({x: x, y: y, width: newWidget.width, height: newWidget.height});
                }
            }
        }

        // console.log('Available Node: ', availableNode);
