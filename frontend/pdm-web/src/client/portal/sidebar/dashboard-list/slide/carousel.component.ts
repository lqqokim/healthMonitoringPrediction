...
import { LinkedList } from './util/linked-list.class';
 ...
 */
@Component({
    selector: 'div[dashboard-carousel]',
    template: `
        <div class="widget-container carousel slide">
            <div class="carousel-inner" (mouseover)="play()" (mouseout)="pause()" title="Auto Sliding">
                <div dashboard-slide *ngFor="let widgetz of widgets" [widgets]="widgetz"></div>
                <div dashboard-slide *ngIf="widgets.length == 0" [widgets]="widgetz"></div>
            </div>
            <div class="widget-footer" *ngIf="widgets.length > 1">
                <ul class="widget-pagenation">
                    <li *ngFor="let slidez of slides; let i = index;" [class.active]="slidez?.active === true" style="cursor: pointer;" (click)="selectSlide(i)"></li>
                </ul>
            </div>
            <div class="widget-footer" *ngIf="widgets.length <= 1">
                <ul class="widget-pagenation">
...
export class CarouselComponent implements OnDestroy {
    ...
    _widgets: any = [];
    @Input() public set widgets(value: any) {
        if (value && value.length > 0) {
            let icons: any = [];
            let length: number = value.length;
            for (let idx = 0; idx < length; idx++) {
                const widget = value[idx];
                icons.push(widget);
                if (idx > 0 && idx % 8 === 0) {
                    this._widgets.push(icons.slice());
                    icons = [];
                }
                if (length < 8 && idx === (length-1)) {
                    this._widgets.push(icons.slice());
                }
            }
