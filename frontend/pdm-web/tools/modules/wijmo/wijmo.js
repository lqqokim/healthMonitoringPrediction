    ...
    function showPopup(popup, ref, above, fadeIn, copyStyles) {
        ...
        if (ref && parent) {
            if (parent == document.body) {
                var pinchZoom = document.documentElement.clientHeight / window.innerHeight;
                if (pinchZoom > 1.0001) {
                    if (ref.clientX != null && ref.clientY != null && ref.pageX != null && ref.pageY != null) {
                        pos.x = ref.clientX;
                        pos.y = ref.clientY;
                    }
                    ref = null;
                    ...
                    ptOffset = new Point(pageXOffset / pinchZoom, pageYOffset / pinchZoom);
                }
            }
            else {
                var brp = parent.getBoundingClientRect();
                ptOffset = new Point(parent.scrollLeft - brp.left, parent.scrollTop - brp.top);
            }
        }
        var css = {
            position: ref ? 'absolute' : 'fixed',
